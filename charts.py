...
class HistogramDockWidget(QDockWidget):
    ...
    def showHistogram(self, image: np.ndarray, modes: AddColorMode = AddColorMode.MIX):
        '''
        显示直方图
        ---
        ...
        if modes == AddColorMode.OVERLAP:
            ...
            def mixColor(c1=(0, 0, 0), c2=(0, 0, 0), c3=(0, 0, 0)):
                '''混合 RGB 颜色'''
                return np.clip(tuple(c1[i] + c2[i] + c3[i] for i in range(3)), 0, 255)
